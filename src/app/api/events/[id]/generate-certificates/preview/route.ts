...
export async function POST(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    const { fields, templateSize } = await request.json()
    ...
    // Draw fields (pakai data dummy)
    for (const f of fields) {
      if (f.key === 'token' && !f.active) continue
      let value = f.label === 'Nama Peserta' ? 'Contoh Nama Peserta' : f.label
      if (f.key === 'number') value = `NOMOR : 1${params.id}/${eventSlug}/${mmRomawi}/${yyyy}`

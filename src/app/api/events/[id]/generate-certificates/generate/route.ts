...
export async function POST(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const [participants] = await db.execute(`SELECT p.*, t.token, t.id as ticket_id, e.name as event_name FROM participants p JOIN tickets t ON p.ticket_id = t.id JOIN events e ON t.event_id = e.id WHERE t.event_id = ? AND t.is_verified = TRUE`, [params.id])
    ...
    for (const cert of oldCertsAll as any[]) {
      if (cert.path) {
        try { await fs.unlink(path.join(process.cwd(), 'public', cert.path)) } catch {}
      }
    }
    ...
    return NextResponse.json({ message: 'Sertifikat berhasil digenerate untuk semua peserta tervalidasi!' })
  } catch (e) {
...
export async function PATCH(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    return NextResponse.json({ message: 'Sertifikat berhasil digenerate untuk peserta yang belum punya sertifikat!' })
  } catch (e) {
